class CustomComponent extends HTMLElement {
 ...
 themes = this.getAttribute('themes')
  constructor() {
    ...
    buttonContainer.innerHTML = this.themes.split(',').map(theme => `<button id="${theme}" class="theme-btn ${theme}" >${theme}</button>`).join('')
    ...
    style.textContent = `
      :host {
          ...
          top: 33px;
          right: 33px;
          // use these variables to change icon colors
